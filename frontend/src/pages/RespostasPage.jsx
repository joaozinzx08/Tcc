  ...
  useEffect(() => {
    async function loadRespostas() {
      ...
      try {
        ...
        // só mostra reclamações que já têm pelo menos uma resposta
        const comResposta = data.filter(
          (c) => c.ComplaintResponses && c.ComplaintResponses.length > 0
        )
        setComplaints(comResposta)
      } catch (err) {
        setError(err.message || 'Não foi possível carregar as respostas.')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="min-h-screen bg-paper text-ink">
      <section className="mx-auto max-w-3xl px-6 py-12">
        <h1 className="text-3xl font-extrabold tracking-tight text-ink">Respostas</h1>
        ...
        {loading && (
          <div className="mt-8 rounded-xl border border-teal-100 bg-white p-10 text-center">
            <p className="text-sm text-ink/55">Carregando respostas…</p>
          </div>
        )}
        ...
        {!loading && !error && complaints.length === 0 && (
          <div className="mt-8 rounded-xl border border-teal-100 bg-white p-10 text-center">
            <p className="text-sm text-ink/55">Nenhuma reclamação sua tem resposta ainda.</p>
          </div>
        )}
        ...
        <div className="mt-6 space-y-3">
          {complaints.map((complaint) => {
            const ultimaResposta =
              complaint.ComplaintResponses[complaint.ComplaintResponses.length - 1]
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <h2 className="mt-0.5 font-semibold text-ink">{complaint.titulo}</h2>
                  </div>
                ...
                <div className="mt-3 rounded-lg bg-teal-50/60 p-3">
                  <p className="text-xs font-medium text-ink/50">
                    {ultimaResposta.Employee?.nome || 'RH'} respondeu:
                  </p>
